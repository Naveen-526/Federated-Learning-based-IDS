...
dataset=pd.read_csv("combined_sensor_dataset.csv")
...
conv_1 = keras.layers.Conv1D(filters=64,kernel_size=5,activation='relu', padding='same')(input_layer)
conv_1 = keras.layers.MaxPool1D(pool_size=2)(conv_1)       
conv_2 = keras.layers.Conv1D(filters=32, kernel_size=5, activation='relu', padding='same')(conv_1)
conv_2 = keras.layers.MaxPool1D(pool_size=2)(conv_2)      
flatten_layer = keras.layers.Flatten()(conv_2)
fully_connected_layer = keras.layers.Dense(256,activation='relu')(flatten_layer)          
...
NAME = "LeNet on IoT Combined Dataset"
